<template >
    <PagerTable :loading="tableDataLoading" :pager-state="pagerState" :add-permiss-code="permiss.add" @list-page="listPage"
        ...
        <template #queryFormBtn>
            <el-button type="info" icon="CaretRight" v-permiss="permiss.terminalTree"
                @click="toTerminalTree()">终端</el-button>
        </template>
        ...
        <el-table-column label="操作"
            ...
            <template #default="scope">
                ...
                <el-button type="info" :loading="scope.row.loading" icon="CaretRight" v-permiss="permiss.terminalSingle"
                    @click="toTerminalSingle(scope.row)">终端</el-button>
            </template>
        </el-table-column>
    </PagerTable>
    ...
    <ElDialog v-model="editDialog.visible" :title="editDialog.title" width="35%">
        <ElForm ref="editFormRef" :model="editFormData" status-icon :rules="editForm.rules" label-width="80px">
                ...
                <el-col :span="18" v-if="!editFormData.use_key">
                    <ElFormItem label="密码" prop="password">
                        <el-input :disabled="!editDialog.showPassword" class="el-password"
                            :type="editDialog.showPassword ? 'text' : 'password'" v-model="editFormData.password"
                            placeholder="请输入密码">
                            <template #append>
            ...
            <ElFormItem v-if="editFormData.use_key" label="私钥" prop="private_key">
                <el-input v-if="editDialog.showPassword" v-model="editFormData.private_key"
                    :autosize="{ minRows: 4, maxRows: 8 }" type="textarea" placeholder="私钥，通常位于 $HOME/.ssh/id_rsa">
                </el-input>
            ...
            <ElFormItem label="描述" prop="desc"> <el-input v-model="editFormData.desc" :autosize="{ minRows: 2, maxRows: 4 }"
                    type="textarea" placeholder="描述内容..." />
            </ElFormItem>
        </ElForm>
        <template #footer>
...
import { IHost, addApi, editApi, listPageApi, getApi, deleteApi, getPasswordAndPrivateKeyApi, testSSHApi } from '@/api/resource/host'
...
const state = reactive<IPageState<IHost & { ssh_uri?: string }> & { editDialog: { showPassword: boolean, hostGroupTreeSelectData: ITreeSelectData[] } }>({
    editFormData: {
        name: '',
        host: '',
        port: 22,
